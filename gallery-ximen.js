/*
 * Ximen After Dark — Gallery configuration
 *
 * Add photos: place images in /images/ximen/ and add entries below.
 * Set tall: true for portrait-orientation images that should span 2 rows in the grid.
 */
const photos = [
  // { src: 'images/ximen/01.jpg', caption: 'Ximending', tall: true },
  // { src: 'images/ximen/02.jpg', caption: 'Ximending' },
];
...
function buildGrid() {
  grid.innerHTML = '';
  if (photos.length === 0) {
    grid.innerHTML = '<div class="empty-gallery">Images coming soon.</div>';
    return;
  }
...
function nextPhoto() { showSingle(current + 1); }
function prevPhoto() { showSingle(current - 1); }
...
prevLink.addEventListener('click', function (e) { e.preventDefault(); prevPhoto(); });
nextLink.addEventListener('click', function (e) { e.preventDefault(); nextPhoto(); });
thumbsLink.addEventListener('click', function (e) { e.preventDefault(); showGrid(); });
